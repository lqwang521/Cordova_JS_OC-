...
#import "HTMICDVIntentAndNavigationFilter.h"
#import "HTMICDV.h"

@interface HTMICDVIntentAndNavigationFilter ()

@property (nonatomic, readwrite) NSMutableArray* allowIntents;
@property (nonatomic, readwrite) NSMutableArray* allowNavigations;
@property (nonatomic, readwrite) HTMICDVWhitelist* allowIntentsWhitelist;
@property (nonatomic, readwrite) HTMICDVWhitelist* allowNavigationsWhitelist;

@end

@implementation HTMICDVIntentAndNavigationFilter
...
{
    self.allowIntentsWhitelist = [[HTMICDVWhitelist alloc] initWithArray:self.allowIntents];
    self.allowNavigationsWhitelist = [[HTMICDVWhitelist alloc] initWithArray:self.allowNavigations];
}
...
#pragma mark HTMICDVPlugin

- (void)pluginInitialize
{
    if ([self.viewController isKindOfClass:[HTMICDVViewController class]]) {
        [(HTMICDVViewController*)self.viewController parseSettingsWithParser:self];
    }
}

+ (HTMICDVIntentAndNavigationFilterValue) filterUrl:(NSURL*)url intentsWhitelist:(HTMICDVWhitelist*)intentsWhitelist navigationsWhitelist:(HTMICDVWhitelist*)navigationsWhitelist
{
    ...
    if (allowNavigationsPass && allowIntentPass) {
        return HTMICDVIntentAndNavigationFilterValueNavigationAllowed;
    } else if (allowNavigationsPass) {
        return HTMICDVIntentAndNavigationFilterValueNavigationAllowed;
    } else if (allowIntentPass) {
        return HTMICDVIntentAndNavigationFilterValueIntentAllowed;
    }
    
    return HTMICDVIntentAndNavigationFilterValueNoneAllowed;
}

- (HTMICDVIntentAndNavigationFilterValue) filterUrl:(NSURL*)url
{
...
+ (BOOL)shouldOverrideLoadWithRequest:(NSURLRequest*)request navigationType:(UIWebViewNavigationType)navigationType filterValue:(HTMICDVIntentAndNavigationFilterValue)filterValue
{
    ...
    switch (filterValue) {
        case HTMICDVIntentAndNavigationFilterValueNavigationAllowed:
            return YES;
        case HTMICDVIntentAndNavigationFilterValueIntentAllowed:
            ...
            return NO;
        case HTMICDVIntentAndNavigationFilterValueNoneAllowed:
