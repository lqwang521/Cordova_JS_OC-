...
#import "HTMICDVUIWebViewEngine.h"
...
@interface HTMICDVUIWebViewEngine ()

@property (nonatomic, strong, readwrite) UIView* engineWebView;
@property (nonatomic, strong, readwrite) id <UIWebViewDelegate> uiWebViewDelegate;
@property (nonatomic, strong, readwrite) HTMICDVUIWebViewNavigationDelegate* navWebViewDelegate;

@end

@implementation HTMICDVUIWebViewEngine
...
- (void)pluginInitialize
{
    ...
    if ([self.viewController conformsToProtocol:@protocol(UIWebViewDelegate)]) {
        self.uiWebViewDelegate = [[HTMICDVUIWebViewDelegate alloc] initWithDelegate:(id <UIWebViewDelegate>)self.viewController];
        uiWebView.delegate = self.uiWebViewDelegate;
    } else {
        self.navWebViewDelegate = [[HTMICDVUIWebViewNavigationDelegate alloc] initWithEnginePlugin:self];
        self.uiWebViewDelegate = [[HTMICDVUIWebViewDelegate alloc] initWithDelegate:self.navWebViewDelegate];
        uiWebView.delegate = self.uiWebViewDelegate;
    }
...
- (void)updateWithInfo:(NSDictionary*)info
{
    ...
    id <UIWebViewDelegate> uiWebViewDelegate = [info objectForKey:kHTMICDVWebViewEngineUIWebViewDelegate];
    NSDictionary* settings = [info objectForKey:kHTMICDVWebViewEngineWebViewPreferences];
