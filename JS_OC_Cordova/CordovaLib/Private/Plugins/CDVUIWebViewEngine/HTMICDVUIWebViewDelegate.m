...
@implementation HTMICDVUIWebViewDelegate
        ...
        if (isTopLevelNavigation) {
            ...
            switch (_state) {
                case STATE_WAITING_FOR_LOAD_FINISH:
                    ...
                    if (_loadCount != 1) {
                        NSLog(@"HTMICDVWebViewDelegate: Detected redirect when loadCount=%ld", (long)_loadCount);
                    }
                    break;
                ...
                default:
                    {
                        NSString* description = [NSString stringWithFormat:@"HTMICDVWebViewDelegate: Navigation started when state=%ld", (long)_state];
                        NSLog(@"%@", description);
                        _loadCount = 0;
                        _state = STATE_WAITING_FOR_LOAD_START;
                        if ([_delegate respondsToSelector:@selector(webView:didFailLoadWithError:)]) {
                            NSDictionary* errorDictionary = @{NSLocalizedDescriptionKey : description};
                            NSError* error = [[NSError alloc] initWithDomain:@"HTMICDVUIWebViewDelegate" code:1 userInfo:errorDictionary];
...
- (void)webViewDidStartLoad:(UIWebView*)webView
{
    ...
    switch (_state) {
        ...
        case STATE_WAITING_FOR_LOAD_START:
            if (_loadCount != 0) {
                NSLog(@"HTMICDVWebViewDelegate: Unexpected loadCount in didStart. count=%ld", (long)_loadCount);
            }
            fireCallback = YES;
            _state = STATE_WAITING_FOR_LOAD_FINISH;
            _loadCount = 1;
        ...
        default:
            NSLog(@"HTMICDVWebViewDelegate: Unexpected didStart with state=%ld loadCount=%ld", (long)_state, (long)_loadCount);
    }
...
- (void)webViewDidFinishLoad:(UIWebView*)webView
{
    ...
    switch (_state) {
        ...
        case STATE_WAITING_FOR_LOAD_START:
            NSLog(@"HTMICDVWebViewDelegate: Unexpected didFinish while waiting for load start.");
            break;
