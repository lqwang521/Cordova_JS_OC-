...
@interface HTMICDVLocalStorage ()

@property (nonatomic, readwrite, strong) NSMutableArray* backupInfo;  // array of HTMICDVBackupInfo objects
@property (nonatomic, readwrite, weak) id <UIWebViewDelegate> webviewDelegate;

@end

@implementation HTMICDVLocalStorage
...
+ (NSMutableArray*)createBackupInfoWithTargetDir:(NSString*)targetDir backupDir:(NSString*)backupDir targetDirNests:(BOOL)targetDirNests backupDirNests:(BOOL)backupDirNests rename:(BOOL)rename
{
    ...
    backupItem = [[HTMICDVBackupInfo alloc] init];
...
+ (BOOL)copyFrom:(NSString*)src to:(NSString*)dest error:(NSError* __autoreleasing*)error
{
    ...
    if (![fileManager fileExistsAtPath:src]) {
        ...
        if (error != NULL) {
            (*error) = [NSError errorWithDomain:kHTMICDVLocalStorageErrorDomain
                                           code:kHTMICDVLocalStorageFileOperationError
...
- (BOOL)shouldBackup
{
    for (HTMICDVBackupInfo* info in self.backupInfo) {
...
- (void)backup:(HTMICDVInvokedUrlCommand*)command
{
    NSString* callbackId = command.callbackId;

    NSError* __autoreleasing error = nil;
    HTMICDVPluginResult* result = nil;
        ...
        if ([info shouldBackup]) {
            ...
            if (callbackId) {
                if (error == nil) {
                    ...
                    result = [HTMICDVPluginResult resultWithStatus:HTMICDVCommandStatus_OK messageAsString:message];
                    [self.commandDelegate sendPluginResult:result callbackId:callbackId];
                } else {
                    message = [NSString stringWithFormat:@"Error in HTMICDVLocalStorage (%@) backup: %@", info.label, [error localizedDescription]];
                    NSLog(@"%@", message);

                    result = [HTMICDVPluginResult resultWithStatus:HTMICDVCommandStatus_ERROR messageAsString:message];
                    [self.commandDelegate sendPluginResult:result callbackId:callbackId];
                }
            }
        }
...
- (void)restore:(HTMICDVInvokedUrlCommand*)command
{
    NSError* __autoreleasing error = nil;
    HTMICDVPluginResult* result = nil;
    NSString* message = nil;
    ...
    for (HTMICDVBackupInfo* info in self.backupInfo) {
        if ([info shouldRestore]) {
            ...
            if (error == nil) {
                ...
                message = [NSString stringWithFormat:@"Error in HTMICDVLocalStorage (%@) restore: %@", info.label, [error localizedDescription]];
                NSLog(@"%@", message);
...
+ (void)__restoreLegacyDatabaseLocationsWithBackupType:(NSString*)backupType
{
    ...
    for (HTMICDVBackupInfo* info in backupInfo) {
...
- (void)onResignActive
{
    ...
    if (exitsOnSuspend) {
        ...
        HTMICDVLocalStorage __weak* weakSelf = self;
...
#pragma mark -
#pragma mark HTMICDVBackupInfo implementation

@implementation HTMICDVBackupInfo
