...
@implementation HTMICDVInvokedUrlCommand
...
+ (HTMICDVInvokedUrlCommand*)commandFromJson:(NSArray*)jsonEntry
{
    return [[HTMICDVInvokedUrlCommand alloc] initFromJson:jsonEntry];
}
...
- (void)massageArguments
{
    ...
    for (NSUInteger i = 0, count = [_arguments count]; i < count; ++i) {
        ...
        NSString* type = [dict objectForKey:@"HTMICDVType"];
