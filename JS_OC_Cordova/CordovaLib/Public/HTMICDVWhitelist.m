...
#import "HTMICDVWhitelist.h"

NSString* const kHTMICDVDefaultWhitelistRejectionString = @"ERROR whitelist rejection: url='%@'";
NSString* const kHTMICDVDefaultSchemeName = @"HTMICDV-default-scheme";

@interface HTMICDVWhitelistPattern : NSObject {
    @private
...
@implementation HTMICDVWhitelistPattern
...
- (id)initWithScheme:(NSString*)scheme host:(NSString*)host port:(NSString*)port path:(NSString*)path
{
    ...
    if (self) {
        if ((scheme == nil) || [scheme isEqualToString:@"*"]) {
            ...
            _scheme = [NSRegularExpression regularExpressionWithPattern:[HTMICDVWhitelistPattern regexFromPattern:scheme allowWildcards:NO] options:NSRegularExpressionCaseInsensitive error:nil];
        }
        if ([host isEqualToString:@"*"] || host == nil) {
            _host = nil;
        } else if ([host hasPrefix:@"*."]) {
            _host = [NSRegularExpression regularExpressionWithPattern:[NSString stringWithFormat:@"([a-z0-9.-]*\\.)?%@", [HTMICDVWhitelistPattern regexFromPattern:[host substringFromIndex:2] allowWildcards:false]] options:NSRegularExpressionCaseInsensitive error:nil];
        } else {
            _host = [NSRegularExpression regularExpressionWithPattern:[HTMICDVWhitelistPattern regexFromPattern:host allowWildcards:NO] options:NSRegularExpressionCaseInsensitive error:nil];
        }
        ...
        if ((path == nil) || [path isEqualToString:@"/*"]) {
            ...
            _path = [NSRegularExpression regularExpressionWithPattern:[HTMICDVWhitelistPattern regexFromPattern:path allowWildcards:YES] options:0 error:nil];
        }
    }
    return self;
}
...
@implementation HTMICDVWhitelist
...
- (id)initWithArray:(NSArray*)array
{
    ...
    if (self) {
        ...
        self.whitelistRejectionFormatString = kHTMICDVDefaultWhitelistRejectionString;
    ...
    if ([origin isEqualToString:@"*"]) {
        ...
        if (m != nil) {
            ...
            if (scheme == nil) {
                ...
                [self.whitelist addObject:[[HTMICDVWhitelistPattern alloc] initWithScheme:@"http" host:host port:port path:path]];
                [self.whitelist addObject:[[HTMICDVWhitelistPattern alloc] initWithScheme:@"https" host:host port:port path:path]];
            } else {
                [self.whitelist addObject:[[HTMICDVWhitelistPattern alloc] initWithScheme:scheme host:host port:port path:path]];
            }
...
- (BOOL)URLIsAllowed:(NSURL*)url logFailure:(BOOL)logFailure
{
    ...
    // http[s] and ftp[s] should also validate against the common set in the kHTMICDVDefaultSchemeName list
    if ([scheme isEqualToString:@"http"] || [scheme isEqualToString:@"https"] || [scheme isEqualToString:@"ftp"] || [scheme isEqualToString:@"ftps"]) {
        NSURL* newUrl = [NSURL URLWithString:[NSString stringWithFormat:@"%@://%@%@", kHTMICDVDefaultSchemeName, [url host], [[url path] stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding]]];
    ...
    for (HTMICDVWhitelistPattern* p in self.whitelist) {
        if ([p matches:url]) {
            return YES;
        }
    }
