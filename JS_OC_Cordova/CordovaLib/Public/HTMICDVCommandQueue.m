...
@interface HTMICDVCommandQueue () {
    NSInteger _lastCommandQueueFlushRequestId;
    __weak HTMICDVViewController* _viewController;
    NSMutableArray* _queue;
    NSTimeInterval _startExecutionTime;
}
@end

@implementation HTMICDVCommandQueue
...
- (id)initWithViewController:(HTMICDVViewController*)viewController
{
    self = [super init];
    if (self != nil) {
...
- (void)enqueueCommandBatch:(NSString*)batchJSON
{
    if ([batchJSON length] > 0) {
        ...
        if ([batchJSON length] < JSON_SIZE_FOR_MAIN_THREAD) {
            [commandBatchHolder addObject:[batchJSON HTMICDV_JSONObject]];
        } else {
            dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^() {
                NSMutableArray* result = [batchJSON HTMICDV_JSONObject];
...
{
    __weak HTMICDVCommandQueue* weakSelf = self;
    ...
    [_viewController.webViewEngine evaluateJavaScript:js
        ...
        if ((error == nil) && [obj isKindOfClass:[NSString class]]) {
            ...
            HTMICDV_EXEC_LOG(@"Exec: Flushed JS->native queue (hadCommands=%d).", [queuedCommandsJSON length] > 0);
        ...
        while ([_queue count] > 0) {
            ...
            while ([commandBatch count] > 0) {
                @autoreleasepool {
                    ...
                    NSArray* jsonEntry = [commandBatch HTMICDV_dequeue];
                    if ([commandBatch count] == 0) {
                        [_queue removeObjectAtIndex:0];
                    }
                    HTMICDVInvokedUrlCommand* command = [HTMICDVInvokedUrlCommand commandFromJson:jsonEntry];
                    HTMICDV_EXEC_LOG(@"Exec(%@): Calling %@.%@", command.callbackId, command.className, command.methodName);

                    if (![self execute:command]) {
#ifdef DEBUG
                            NSString* commandJson = [jsonEntry HTMICDV_JSONString];
...
- (BOOL)execute:(HTMICDVInvokedUrlCommand*)command
{
    ...
    HTMICDVPlugin* obj = [_viewController.commandDelegate getCommandInstance:command.className];

    if (!([obj isKindOfClass:[HTMICDVPlugin class]])) {
        NSLog(@"ERROR: Plugin '%@' not found, or is not a HTMICDVPlugin. Check your plugin mapping in config.xml.", command.className);
        return NO;
    }
    BOOL retVal = YES;
