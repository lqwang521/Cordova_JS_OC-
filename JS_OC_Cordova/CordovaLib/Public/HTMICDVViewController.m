...
#import "HTMICDVUIWebViewDelegate.h"
#import "HTMICDVConfigParser.h"
#import "HTMICDVUserAgentUtil.h"
#import <AVFoundation/AVFoundation.h>
#import "NSDictionary+CordovaPreferences.h"
#import "HTMICDVLocalStorage.h"
#import "HTMICDVCommandDelegateImpl.h"

@interface HTMICDVViewController () {
...
@property (nonatomic, readwrite, strong) id <HTMICDVWebViewEngineProtocol> webViewEngine;
...
@implementation HTMICDVViewController
...
- (void)__init
{
    if ((self != nil) && !self.initialized) {
        _commandQueue = [[HTMICDVCommandQueue alloc] initWithViewController:self];
        _commandDelegate = [[HTMICDVCommandDelegateImpl alloc] initWithViewController:self];
...
- (void)printVersion
{
    NSLog(@"Apache Cordova native platform version %@ is starting.", HTMICDV_VERSION);
}
...
- (void)loadSettings
{
    HTMICDVConfigParser* delegate = [[HTMICDVConfigParser alloc] init];
...
- (void)viewDidLoad
{
    ...
    [HTMICDVLocalStorage __fixupDatabaseLocationsWithBackupType:backupWebStorageType];
    ...
    /*
     * Fire up HTMICDVLocalStorage to work-around WebKit storage limitations: on all iOS 5.1+ versions for local-only backups, but only needed on iOS 5.1 for cloud backup.
    ...
    if ([self.startupPluginNames count] > 0) {
        [HTMICDVTimer start:@"TotalPluginStartup"];

        for (NSString* pluginName in self.startupPluginNames) {
            [HTMICDVTimer start:pluginName];
            [self getCommandInstance:pluginName];
            [HTMICDVTimer stop:pluginName];
        }

        [HTMICDVTimer stop:@"TotalPluginStartup"];
    }

    // /////////////////
    NSURL* appURL = [self appUrl];

    [HTMICDVUserAgentUtil acquireLock:^(NSInteger lockToken) {
        _userAgentLockToken = lockToken;
        [HTMICDVUserAgentUtil setUserAgent:self.userAgent lockToken:lockToken];
        if (appURL) {
...
-(void)viewWillAppear:(BOOL)animated
{
    ...
    [[NSNotificationCenter defaultCenter] postNotification:[NSNotification notificationWithName:HTMICDVViewWillAppearNotification object:nil]];
}

-(void)viewDidAppear:(BOOL)animated
{
    [super viewDidAppear:animated];
    [[NSNotificationCenter defaultCenter] postNotification:[NSNotification notificationWithName:HTMICDVViewDidAppearNotification object:nil]];
}

-(void)viewWillDisappear:(BOOL)animated
{
    [super viewWillDisappear:animated];
    [[NSNotificationCenter defaultCenter] postNotification:[NSNotification notificationWithName:HTMICDVViewWillDisappearNotification object:nil]];
}

-(void)viewDidDisappear:(BOOL)animated
{
    [super viewDidDisappear:animated];
    [[NSNotificationCenter defaultCenter] postNotification:[NSNotification notificationWithName:HTMICDVViewDidDisappearNotification object:nil]];
}

-(void)viewWillLayoutSubviews
{
    [super viewWillLayoutSubviews];
    [[NSNotificationCenter defaultCenter] postNotification:[NSNotification notificationWithName:HTMICDVViewWillLayoutSubviewsNotification object:nil]];
}

-(void)viewDidLayoutSubviews
{
    [super viewDidLayoutSubviews];
    [[NSNotificationCenter defaultCenter] postNotification:[NSNotification notificationWithName:HTMICDVViewDidLayoutSubviewsNotification object:nil]];
}

-(void)viewWillTransitionToSize:(CGSize)size withTransitionCoordinator:(id<UIViewControllerTransitionCoordinator>)coordinator
{
    [super viewWillTransitionToSize:size withTransitionCoordinator:coordinator];
    [[NSNotificationCenter defaultCenter] postNotification:[NSNotification notificationWithName:HTMICDVViewWillTransitionToSizeNotification object:[NSValue valueWithCGSize:size]]];
}
...
- (UIView*)newCordovaViewWithFrame:(CGRect)bounds
{
    ...
    if (!defaultWebViewEngineClass) {
        defaultWebViewEngineClass = @"HTMICDVUIWebViewEngine";
    }
    ...
    if (NSClassFromString(webViewEngineClass)) {
        ...
        if (!self.webViewEngine || ![self.webViewEngine conformsToProtocol:@protocol(HTMICDVWebViewEngineProtocol)] || ![self.webViewEngine canLoadRequest:[NSURLRequest requestWithURL:self.appUrl]]) {
    ...
    if ([self.webViewEngine isKindOfClass:[HTMICDVPlugin class]]) {
        [self registerPlugin:(HTMICDVPlugin*)self.webViewEngine withClassName:webViewEngineClass];
    }
...
- (NSString*)userAgent
{
    ...
    if (self.baseUserAgent != nil) {
        ...
        localBaseUserAgent = [HTMICDVUserAgentUtil originalUserAgent];
    }
...
- (void)viewDidUnload
{
    ...
    [HTMICDVUserAgentUtil releaseLock:&_userAgentLockToken];
    ...
    if (!obj) {
        @try {
            ...
            if (obj != nil) {
                ...
                NSLog(@"HTMICDVPlugin class %@ (pluginName: %@) does not exist.", className, pluginName);
            }
