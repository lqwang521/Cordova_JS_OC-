...
@implementation HTMICDVCommandDelegateImpl
...
{
    HTMICDV_EXEC_LOG(@"Exec: evalling: %@", [js substringToIndex:MIN([js length], 160)]);
    [_viewController.webViewEngine evaluateJavaScript:js completionHandler:^(id obj, NSError* error) {
        ...
        if ([obj isKindOfClass:[NSString class]]) {
            ...
            if ([commandsJSON length] > 0) {
                HTMICDV_EXEC_LOG(@"Exec: Retrieved new exec messages by chaining.");
            }
...
- (void)sendPluginResult:(HTMICDVPluginResult*)result callbackId:(NSString*)callbackId
{
    HTMICDV_EXEC_LOG(@"Exec(%@): Sending result. Status=%@", callbackId, result.status);
